    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                <h4 class="mb-4">Cadastrar Novo Medicamento</h4>
                ...
                <form action="{{ route('medicamento.store') }}" method="POST">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" name="nome_cientifico" class="form-control" 
                               value="{{ old('nome_cientifico') }}" required>
                    </div>

                    <div class="form-group mb-3">
                        <label for="nome_comercial">Nome Comercial</label>
                        <input type="text" name="nome_comercial" class="form-control" 
                               value="{{ old('nome_comercial') }}" required>
                    </div>

                    <div class="form-group mb-3">
                        <label for="finalidade">Finalidade</label>
                        <textarea name="finalidade" class="form-control" rows="2" required>{{ old('finalidade') }}</textarea>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="forma_administracao" class="form-control" required>
                            ...
                            <option value="Comprimido" {{ old('forma_administracao') == 'Comprimido' ? 'selected' : '' }}>Comprimido</option>
                            <option value="Xarope" {{ old('forma_administracao') == 'Xarope' ? 'selected' : '' }}>Xarope</option>
                            <option value="Injeção" {{ old('forma_administracao') == 'Injeção' ? 'selected' : '' }}>Injeção</option>
                            <option value="Pomada" {{ old('forma_administracao') == 'Pomada' ? 'selected' : '' }}>Pomada</option>
                            <option value="Inalador" {{ old('forma_administracao') == 'Inalador' ? 'selected' : '' }}>Inalador</option>
                            <option value="Outro" {{ old('forma_administracao') == 'Outro' ? 'selected' : '' }}>Outro</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" name="dosagem_padrao" class="form-control" 
                               value="{{ old('dosagem_padrao') }}" required>
                    </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="number" name="quantidade_estoque" class="form-control" 
                               value="{{ old('quantidade_estoque', 0) }}" min="0" required>
                    </div>
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('medicamento.index') }}" class="btn btn-secondary">Cancelar</a>
